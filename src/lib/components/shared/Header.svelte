...
<header class:is_header_open={is_open} data-aos="fade-down" data-aos-duration="1300">
    <div class="header px-sm-3 px-0 py-1">
        <div class="row justify-content-between align-items-center px-3 mx-0">
            <div class="col px-0 lang-2">
    ...
    <div transition:fly={{ y: -500, duration: 1000 }} class="header-mobile bg-yellow">
        <div class="menu-mobile row mx-0 justify-contnent-center align-items-center">
            <ul class="menu-list-mobile px-0">
                {#each $lang=='es'?menu_options.es:menu_options.en as option,i}
                <li class:active={path==option.link} ><a href="{option.link}" on:click={()=>is_open = false} class="menu-link">{option.name}</a></li>
                {/each}
                 <li class="mt-4"><a href="/contacto" on:click={()=>is_open = false} class="btn btn-pink">{$lang=='es'?'Contáctame':'Get in touch'}</a></li>
            </ul>
             <ul class="menu-list-socials py-4 text-center">
                <li class="mb-2"><a href="https://wa.me/524491984582" target="_blank" class="social-link"><i class="bi bi-whatsapp"></i> (+52) 449 198 45 82</a></li>
                <li><a href="mailto:paolagtzsal@gmail.com" target="_blank" class="social-link"><i class="bi bi-envelope"></i> paolagtzsal@gmail.com</a></li>
             </ul>
        </div>
    </div>
    {/if}
...
<style>
    .header {position: relative; background: var(--yellow); border-bottom: solid 1px rgba(0, 0, 0, 0.441);}
    .menu{display: flex;}
    ...
    .header-mobile{position: fixed; width: 100%; height: 100vh; top: 0;z-index: 1; border-bottom: solid 1px rgba(0, 0, 0, 0.846);}
    ...
    .menu-mobile{height: 90vh; width: 100%; position: relative;}
    .menu-list-mobile{font-size: 2.3rem; font-weight: 200; text-align: center; margin-bottom: 5em;}
    .menu-list-mobile li{margin: 15px 10px;}
    .menu-list-mobile li.active{font-weight: 400;}
    .lang-2{display: block; position: absolute;}
    .menu-list-socials{position: fixed; bottom: 0%; background-color: var(--mint); font-weight: bold;} 
    .menu-list-socials i{font-size: 20px;}
    }
